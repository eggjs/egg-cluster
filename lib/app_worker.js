...
if (cluster.worker) {
  ...
  cluster.worker.once('disconnect', () => {
    consoleLogger.warn('[app_worker] receive disconnect event in cluster fork mode, exitedAfterDisconnect:%s',
      !!cluster.worker.exitedAfterDisconnect);
  });
} else {
  // child_process mode
  process.once('disconnect', () => {
    consoleLogger.error('[app_worker] receive disconnect event on child_process fork mode, exiting with code:1');
    process.exit(1);
  });
}
