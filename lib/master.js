...
class Master extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.logger[this.logMethod](`[master] =================== ${frameworkPkg.name} start =====================`);
    ...
    if (this.isProduction) {
      ...
      this.logger[this.logMethod]('[master] start with options: %j', this.options);
    }
    this.logger[this.logMethod]('[master] start with env: isProduction: %s, isDebug: %s, EGG_SERVER_ENV: %s, NODE_ENV: %s',
  ...
  forkAgentWorker() {
    ...
    this.logger[this.logMethod]('[master] agent_worker#%s:%s start with clusterPort:%s',
  ...
  forkAppWorkers() {
    ...
    const args = [ JSON.stringify(this.options) ];
    this.logger[this.logMethod]('Start appWorker with args %j', args);
    cfork({
      exec: appWorkerFile,
      args,
      silent: false,
    ...
    cluster.on('fork', worker => {
      ...
      this.logger[this.logMethod]('[master] app_worker#%s:%s start, state: %s, current workers: %j',
  ...
  killAgentWorker() {
    if (this.agentWorker) {
      this.logger[this.logMethod]('kill agent worker with signal SIGTERM');
  ...
  onAgentExit(data) {
    ...
    if (this.isStarted) {
      this.logger[this.logMethod]('[master] try to start a new agent_worker after 1s ...');
      setTimeout(() => {
  ...
  onAppStart(data) {
    ...
    this.logger[this.logMethod]('[master] app_worker#%s:%s started at %s, remain %s (%sms)',
   ...
   */
  onReload() {
    this.logger[this.logMethod]('[master] reload workers...');
    for (const id in cluster.workers) {
  ...
  close() {
    ...
    this.logger[this.logMethod]('[master] close done, exiting with code:0');
    process.exit(0);
  }
}
