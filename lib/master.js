...
class Master extends EventEmitter {
   ...
   *
   *  - {Boolean | Number} [debug] whether debug worker
   *    - **automatically detect the protocol used by the targeted runtime, 8.0+ the new 'inspector' protocol is used.**
   *    - Boolean: auto detect debug port, default to 9229(inspect) or 5858(debug)
   *    - Number: use as debug port
   *  - {Boolean | Number} [debugAgent] whether debug agent
   *    - Boolean: auto detect debug port, default to 9227(inspect) or 5856(debug)
   *    - Number: use as debug port
   *  - {Boolean} [debugBrk] whether stop at the top of worker initial script
   *  - {Boolean} [debugAgentBrk] whether stop at the top of agent initial script
   */
  constructor(options) {
    super();
    ...
    this.isDebug = isDebug(this.options);
  ...
  forkAgentWorker() {
    ...
    if (this.options.debugAgent) {
      opt.execArgv = process.execArgv.concat(`--${this.options.debugProtocol}=${this.options.debugAgent}`);
      if (this.options.debugAgentBrk) {
        opt.execArgv.push(`--${this.options.debugProtocol}-brk`);
      }
      this.logger.info('[master] agent_worker debug with %j', opt.execArgv);
    }

    const agentWorker = this.agentWorker = childprocess.fork(agentWorkerFile, args, opt);
    agentWorker.id = ++this.agentWorkerIndex;
    this.log('[master] agent_worker#%s:%s start with clusterPort:%s', agentWorker.id, agentWorker.pid, this.options.clusterPort);
  ...
  forkAppWorkers() {
    ...
    if (this.options.debug) {
      // hack, for 6.x and 8.x
      // TODO: use `inspectPort` at egg next major version
      process.debugPort = this.options.debug - 1;
      execArgv = process.execArgv.concat(`--${this.options.debugProtocol}=${this.options.debug}`);
      if (this.options.debugBrk) {
        execArgv.push(`--${this.options.debugProtocol}-brk`);
      }
      this.logger.info('[master] app_worker debug with %j', execArgv);
    }
...
function isDebug(options) {
  return process.execArgv.some(argv => argv.includes('--debug') || argv.includes('--inspect')) || options.debug;
}
