...
class Master extends EventEmitter {
  ...
  forkAgentWorker() {
    ...
    this.logger.info('[master] agent_worker#%s:%s start with %j',
      agentWorker.id, agentWorker.pid, args);
  ...
  onAppStart(data) {
    ...
    this.logger.warn('[master] app_worker#%s:%s started at %s, remain %s (%sms)',
  ...
  onExit() {
    ...
    function receiveSig(sig) {
      this.logger.info('[master] receive signal %s, exiting with code:0', sig);
      process.exit(0);
    }
  }
