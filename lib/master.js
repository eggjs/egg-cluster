...
class Master extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.logger.info('[master] start with env: isProduction: %s, isDebug: %s, EGG_SERVER_ENV: %s, NODE_ENV: %s',
  ...
  forkAgentWorker() {
    ...
    this.logger.info('[master] agent_worker#%s:%s start with clusterPort:%s',
  ...
  forkAppWorkers() {
    ...
    cluster.on('fork', worker => {
      ...
      this.logger.info('[master] app_worker#%s:%s start, state: %s, current workers: %j',
  ...
  onAgentExit(data) {
    ...
    if (this.isStarted) {
      this.logger.warn('[master] try to start a new agent_worker after 1s ...');
      setTimeout(() => {
        this.logger.warn('[master] new agent_worker starting...');
        this.forkAgentWorker();
      }, 1000);
      this.messenger.send({
  ...
  onAppStart(data) {
    ...
    this.logger.info('[master] app_worker#%s:%s started at %s, remain %s (%sms)',
  ...
  onExit() {
    ...
    // kill(2) Ctrl-C
    process.once('SIGINT', receiveSig.bind(this, 'SIGINT'));
    // kill(3) Ctrl-\
    process.once('SIGQUIT', receiveSig.bind(this, 'SIGQUIT'));
    // kill(15) default
    process.once('SIGTERM', receiveSig.bind(this, 'SIGTERM'));

    function receiveSig(sig) {
      if (this.closed) return;

      this.logger.info('[master] receive signal %s, closing', sig);
      this.close();
    }
  }
  ...
  close() {
    ...
    this.logger.info('[master] close done, exiting with code:0');
    process.exit(0);
  }
}
