...
class Master extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (process.platform === 'win32') {
        try {
            require.resolve('windows-window-controller');
            require.resolve('ffi');
            nwwc = require('windows-window-controller');
        }
        catch (ex) {
            // void
        }
  ...
  onAgentStart() {
    ...
    if (nwwc) {
      const title = this.options.title + (this.options.port ? `:${this.options.port}:` : '');
      nwwc.set_title(title, { matchType: 'pid', matchValue: this.agentWorker.pid });
      nwwc.hide({ matchType: 'pid', matchValue: this.agentWorker.pid });
      this.log(`Set worker title as "${title}" and hide window during onAgentStart`);
    }
  }
  ...
  onAppStart(data) {
    ...
    for (const id in cluster.workers) {
      ...
      if (nwwc) {
        const title = this.options.title + (this.options.port ? `:${this.options.port}:` : '');
        nwwc.set_title(title, { matchType: 'pid', matchValue: worker.process.pid });
        nwwc.hide({ matchType: 'pid', matchValue: worker.process.pid });
        this.log('Set worker title as "${title}" and hide window during onAppStart');
      }
    }
  ...
  onReload() {
    ...
    for (const id in cluster.workers) {
      ...
      if (nwwc) {
        ...
        nwwc.set_title(title, {matchType: 'pid', matchValue: worker.process.pid});
        nwwc.hide({ matchType: 'pid', matchValue: worker.process.pid });
        this.log('Set worker title as "${title}" and hide window during onReload');
      }
    }
