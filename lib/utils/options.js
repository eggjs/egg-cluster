...
module.exports = function(options) {
  ...
  const debugProtocol = isNew ? 'inspect' : 'debug';
  ...
  if (options.debug || options.debugBrk) {
    ...
    options.appExecArgv = process.execArgv.concat(`--${debugProtocol}=${options.debug}`);
    if (options.debugBrk) {
      options.appExecArgv.push(`--${debugProtocol}-brk`);
    }
  }
  ...
  if (options.debugAgent || options.debugAgentBrk) {
    ...
    if (options.debugAgent === true) options.debugAgent = defaultDebugPort - 2;

    // normalize to boolean
    options.debugAgentBrk = !!options.debugAgentBrk;

    // calculate execArgv
    options.agentExecArgv = process.execArgv.concat(`--${debugProtocol}=${options.debugAgent}`);
    if (options.debugAgentBrk) {
      options.agentExecArgv.push(`--${debugProtocol}-brk`);
    }
  }

  if (options.debug || options.debugAgent) options.debugProtocol = debugProtocol;
